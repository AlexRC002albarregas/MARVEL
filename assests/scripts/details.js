...
function print_character(ch_data) {
    ...
    if (data.length != 0) {
        ...
        let description = data.description || "No hay una descripción disponible."; // Verifica si hay una descripción

        let name_h1 = `<div id="name_align"><h1 id="char_name">${char_name}</h1><p style="color: #ffff; font-size: 16px;">${description}</p></div>`;

        let comics = data.comics.items;
        let comics_li = `<h3>Cómics</h3><ul>`;
        for (c of comics) {
        ...
        let stories_li = `<h3>Historias</h3><ul>`;
        for (c of stories) {
            let li_ele = `<li>${c.name}</li>`;
            stories_li += li_ele;
        }
        ...
        let events_li = `<h3>Eventos</h3><ul>`;
        for (c of comics) {
            let li_ele = `<li>${c.name}</li>`;
            events_li += li_ele;
        }
