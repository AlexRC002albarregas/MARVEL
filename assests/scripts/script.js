...
function fetchMaster(URL = (getCharacters + limit + offset + keys)) {
    fetch(URL)
...
function print_characters(fetch_characters) {
    ...
    if (results.length != 0) {
        ...
        ${character_thumb}
        <a href="${ele_link}">${character_name}</a>
        ${fav_button}
        </div>`;
        ...
        listHolder.innerHTML = "<h1> Sin resultados encontrados, por favor prueba con otras palabras. </h1>";
    }
